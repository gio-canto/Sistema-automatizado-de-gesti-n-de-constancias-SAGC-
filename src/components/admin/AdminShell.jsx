...
export default function AdminShell({
  ...
  return (
    <section className="apple-admin-shell">
      <aside className="apple-admin-sidebar">
        <div className="apple-admin-sidebar__traffic" aria-hidden="true">
          <i className="red" /><i className="yellow" /><i className="green" />
        </div>

        <button className="apple-admin-sidebar__back" type="button" onClick={onBack}>
          <span>‹</span> Volver a SAGC
        </button>

        <div className="apple-admin-sidebar__brand">
          <div className="apple-admin-sidebar__mark">S</div>
          <div><strong>SAGC</strong><small>Admin Console</small></div>
        </div>
            ...
            <div className="apple-admin-sidebar__group" key={group.label}>
              ...
              {group.items.map(([id, label, icon]) => (
                ...
                >
                  ...
                  <span>{label}</span>
                  {id === 'security' && elevated ? <i className="apple-admin-sidebar__dot" /> : null}
                </button>
              ))}
            </div>
          ))}
        ...
        <div className="apple-admin-sidebar__secure-card">
          <div>
            <span className={elevated ? 'is-unlocked' : ''} />
            <strong>{elevated ? 'Modo elevado' : 'Modo protegido'}</strong>
          </div>
          <small>{elevated ? 'Acciones sensibles desbloqueadas temporalmente.' : 'Confirma tu contraseña para acciones sensibles.'}</small>
          <button type="button" onClick={elevated ? onLock : onUnlock}>
            {elevated ? 'Bloquear ahora' : 'Desbloquear'}
          </button>
        </div>
      ...
      <div className="apple-admin-main">
        <header className="apple-admin-topbar">
          <div>
            ...
            <span>Centro de control SAGC</span>
          </div>
